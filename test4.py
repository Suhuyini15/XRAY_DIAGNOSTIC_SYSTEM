...
# Disease knowledge base (same as before)
DISEASE_KNOWLEDGE = {
...
def get_patient_details():
    ...
    for condition in RespiratoryCondition:
        print(f"- {condition.value}")

    condition_input = None
    while condition_input is None:
        cond = input("Enter diagnosed condition: ").lower().strip()
        for c in RespiratoryCondition:
            if cond == c.value:
                condition_input = c
                break
        if condition_input is None:
            print("Invalid condition. Please select from the list.")
